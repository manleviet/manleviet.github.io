---
layout: default
title: Teaching – Viet-Man Le
---

## Teaching

### Courses at Graz University of Technology

**Introduction to Structured Programming** (ISP) — *Summer Semester*, 2020–2026

Position: **University Assistant** — responsible for organizing the course and designing assignments. The course provides a foundational overview of core programming concepts and techniques in the C programming language (nominated for the *Best Teaching Award* in 2023, 2024, and 2025).

**Object-Oriented Analysis and Design** (OAD) — *Winter Semester*, 2022–2026

Position: **University Assistant** — responsible for designing assignments on design principles and design patterns; lecturer on *Design Patterns* in 2024 and 2025. The course provides a foundational introduction to object-oriented software development, covering class diagrams, state charts, object-relational mapping, design patterns, automated testing, and fundamental software design principles.

### Bachelor and Master Thesis Co-supervision

**2025** — Co-supervisor of the Bachelor thesis *"FM2ExConf: A Web-Service to Convert Feature Models into an Executable Representation in Microsoft Excel"* by **Fabian True**. Outcome: the [FM2ExConf](https://fm2exconf.sai.tugraz.at){:target="_blank"} tool.

**2025** — Co-supervisor of the Bachelor thesis *"From Conflicts to Efficiency: Learning Constraint Orderings for QuickXPlain"* by **Nhat Minh Hoang**.

**2025** — Co-supervisor of the Master thesis by **Lukas A. Feldgrill**. Outcome: [*"Robust Lazy Conflict Detection via Multi-Conflict Extraction and Genetic Diversity Control"*](https://ojs.aaai.org/index.php/AAAI/article/view/38995){:target="_blank"} at the *40th AAAI Conference on Artificial Intelligence* (AAAI 2026).

**2023** — Co-supervisor of the Bachelor thesis *"PWipeOutR: Parallelized Redundancy Detection for Feature Models"* by **Yunus Emre Akca**.

**2022** — Co-supervisor of the Bachelor thesis by **Tamim Burgstaller**. Outcome: [*"FMTesting: A FeatureIDE Plug-in for Automated Feature Model Analysis and Diagnosis"*](https://doi.org/10.3233/FAIA230640){:target="_blank"} at the *12th Conference on Prestigious Applications of Intelligent Systems* (PAIS / ECAI 2023).

### Past Teaching at Hue University

Course materials and supervision records in Vietnamese:

- [*Cơ sở Lập trình* (Introduction to Programming)](https://sites.google.com/view/manleviet/giang-day/co-so-lap-trinh){:target="_blank"}
- [*Cơ sở Lập trình* (Introduction to Programming (Python))](https://sites.google.com/view/manleviet/giang-day/co-so-lap-trinh-qtkd){:target="_blank"}
- [*Lập trình Hướng Đối tượng* (Object-Oriented Programming)](https://sites.google.com/view/manleviet/giang-day/lap-trinh-huong-doi-tuong){:target="_blank"}
- [*Hướng dẫn Khóa luận* (Bachelor Thesis Supervision)](https://sites.google.com/view/manleviet/huong-dan-khoa-luan){:target="_blank"}